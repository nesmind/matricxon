"""Cross-checks `Qwen2Architecture`'s forward pass against the real HF `Qwen2.5-0.5B-Instruct`
model - `validate_bert.py`-style single-process comparison (both real checkpoints are small
enough to load directly, no truncation/memory-capped subprocess dance needed, unlike the
`nemotron_h`/M3 oracle checks). Downloads the real small GGUF once (~491MB, cached after) if not
already present locally; the HF reference downloads/caches itself via `transformers`.

This is the real target of this session's real-weight oracle validation: it caught a real bug
(neither q/k weight nor bias needed `unpermute_rope_rows`, contrary to the original assumption -
see `Qwen2Architecture`'s own docstring) that ran without error but produced pure garbage
generation before this check was run. Also verifies real q/k/v bias loads correctly.
"""
...
def main() -> None:
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(HF_REPO, torch_dtype=torch.float32).eval()
