"""Builds a tiny, complete, valid `command-r` GGUF file - mirrors `tiny_gguf_qwen2.py`'s
structure, but with real bias-free `LayerNorm` metadata/tensors (`attn_norm`/`output_norm` have
weight only - no bias tensor at all, confirmed real for this architecture) and a real
`logit_scale` value (non-1.0, so a bug that silently drops the multiply changes the numeric
output, not just shapes).
"""
...
def build_tiny_command_r_gguf(path: Path, seed: int = 0, tied_embeddings: bool = False) -> Path:
    ...
    builder = (
        GGUFBuilder()
        .set_str("general.architecture", "command-r")
        .set_u32("command-r.embedding_length", N_EMBD)
        .set_u32("command-r.attention.head_count", N_HEAD)
        .set_u32("command-r.attention.head_count_kv", N_HEAD_KV)
        .set_u32("command-r.block_count", N_LAYER)
        .set_u32("command-r.feed_forward_length", FFN_LEN)
        .set_f32("command-r.attention.layer_norm_epsilon", 1e-5)
        .set_u32("command-r.vocab_size", vocab_size)
        .set_f32("command-r.rope.freq_base", 10000.0)
        .set_f32("command-r.logit_scale", LOGIT_SCALE)
