"""`/api/chat` against a tiny but real `qwen3` GGUF (see tests/tiny_gguf_qwen3.py) - exercises
QK-norm end to end. Real GGUFModelLoader/Qwen3Architecture/GGUFTokenizer/ChatEngine/NDJSON stack,
not mocked. See scripts/oracle/validate_qwen3.py for the real-weight numeric proof, including the
QK-norm-permutation reasoning `Qwen3Architecture`'s own docstring calls out.
"""
...
class TestQwen3ChatHappyPath:
    def test_streams_ndjson_chunks_ending_in_a_done_line(
        self, client: TestClient, tiny_qwen3_model: InstalledModel
    ) -> None:
        response = client.post(
            "/api/chat",
            json={
                "model": "tiny-qwen3:latest",
...
class TestQwen3ChatTiedEmbeddings:
    def test_streams_ndjson_chunks_ending_in_a_done_line(
        self, client: TestClient, tiny_qwen3_tied_embeddings_model: InstalledModel
    ) -> None:
        response = client.post(
            "/api/chat",
            json={
                "model": "tiny-qwen3-tied:latest",
                "messages": [{"role": "user", "content": "hi"}],
                "options": {"temperature": 0.0, "num_ctx": 32, "num_predict": 5},
            },
        )
