"""`/api/chat` against a tiny but real `command-r` GGUF (see tests/tiny_gguf_command_r.py). Real
GGUFModelLoader/CommandRArchitecture/GGUFTokenizer/ChatEngine/NDJSON stack, not mocked - exercises
the real parallel attention+FFN block, bias-free LayerNorm, and logit_scale multiply. See
scripts/oracle/validate_command_r.py for the real-weight numeric proof.
"""
...
class TestCommandRChatHappyPath:
    def test_streams_ndjson_chunks_ending_in_a_done_line(
        self, client: TestClient, tiny_command_r_model: InstalledModel
    ) -> None:
        response = client.post(
            "/api/chat",
            json={
                "model": "tiny-command-r:latest",
...
class TestCommandRChatTiedEmbeddings:
    def test_streams_ndjson_chunks_ending_in_a_done_line(
        self, client: TestClient, tiny_command_r_tied_embeddings_model: InstalledModel
    ) -> None:
        response = client.post(
            "/api/chat",
            json={
                "model": "tiny-command-r-tied:latest",
                "messages": [{"role": "user", "content": "hi"}],
                "options": {"temperature": 0.0, "num_ctx": 32, "num_predict": 5},
            },
        )
